...
public:
    using StrategyCreator = std::function<std::shared_ptr<OperationStrategy<Type>>()>;// A function that creates a strategy.
    ...
    std::shared_ptr<Expression<Type>> createConstant(Type value) {
        return std::make_shared<Constant<Type>>(value);
    }
    
    // Creates a variable expression.
    // This method returns a shared pointer to an IndependentVariable expression.
    std::shared_ptr<Expression<Type>> createVariable() {
        return std::make_shared<IndependentVariable<Type>>();
    }
...
protected:
    ...
    void registerStrategy(const std::string& strategyType,std::shared_ptr<OperationStrategy<Type>> strategy) {
        strategyCreators[strategyType] = [strategy]() -> std::shared_ptr<OperationStrategy<Type>> {
            return strategy;
        };
    }
